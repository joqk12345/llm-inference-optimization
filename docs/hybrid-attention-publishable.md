...
**状态空间模型的演进**：State Space Models（SSM）有着悠久的控制理论与动力学系统历史，近年来被重新引入 LLM 领域。表 2-3 给出了 SSM 的演进历程。
...
**混合模型格局**：混合模型的核心思路很清晰——保留 Attention 的建模能力，同时插入 Mamba 或线性注意力层来提升长序列效率。
