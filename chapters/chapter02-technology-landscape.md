...
DeepSeek V3 在 2024 年 12 月发布技术报告,展示了大规模 MoE 在训练与推理效率上的可行性。citeturn0search3

**关键事实(官方报告)**  
- 671B 总参数,每 token 激活 37B 参数  citeturn0search3  
- 采用 MoE 架构与多 token 预测等训练策略  citeturn0search3  
- 报告中给出与主流闭源模型可比的结果与训练细节  citeturn0search3  
...
**Prefill-Decode 分离 (PD分离)** 是将提示词处理(Prefill)与逐 token 生成(Decode)拆开的服务模式,适合在负载不均或硬件异构时优化资源利用。citeturn0search4
...
**重要提醒**  
- PD 分离会引入 KV 传输与网络开销,并不保证吞吐量提升  citeturn0search4  
- 某些实现明确指出,该模式更多用于调优时延与资源比例  citeturn0search4  
...
**3. 推理框架的集中化**
- 常见选择包括 vLLM、SGLang、TensorRT-LLM  citeturn0search2turn0search7turn1search1
- 生态、可运维性与可迁移性是关键考虑因素
...
**典型技术**:
- 推理框架: vLLM、SGLang、TensorRT-LLM  citeturn0search2turn0search7turn1search1
...
**典型技术**:
- Continuous Batching (连续批处理)  citeturn0search7
...
**典型技术**:
- KV Cache (标准：复用历史 token 的键值对，避免重复计算)  citeturn1search1
- 前缀缓存 (Prefix Cache：复用共享前缀的计算结果)  citeturn0search7
- 提示词缓存 (Prompt Cache：针对固定系统提示词或模板做缓存)  citeturn0search7
- 结果缓存 (Response Cache：针对重复请求直接返回已生成结果)  citeturn0search7
...
- DeepSeek-V3 Technical Report (arXiv:2412.19437, Dec 2024) citeturn0search3
- Ray Serve LLM: Prefill-Decode Disaggregation citeturn0search4
- vLLM: Disaggregated Prefilling (experimental) citeturn0search2
- SGLang Documentation citeturn0search7
- NVIDIA TensorRT-LLM Documentation citeturn1search1
