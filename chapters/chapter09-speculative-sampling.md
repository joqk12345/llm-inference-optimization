...
这种"投机-验证"的机制,可以将生成速度提升 2-3 倍,而精度损失很小。本章将深入讲解:
- 为什么自回归生成慢,投机采样如何解决
- 投机采样的原理和流程
- 草稿模型的选择策略
- Eagle 3 (NVIDIA 官方支持) 的实战应用
- vLLM Speculators 的端到端训练

**学完本章,你将能够使用投机采样显著提升长文本生成的速度。**
...
单个 forward pass 的时间:
  Llama-2-7B (A100):
    - 单次 forward pass: ~20ms
    - 生成 100 tokens: 100 × 20ms = 2000ms
    - 用户感知: 明显延迟

Llama-2-70B (A100):
    - 单次 forward pass: ~80ms
    - 生成 100 tokens: 100 × 80ms = 8000ms
    - 用户感知: 非常慢
```
...
│ Time:   ~10ms (小模型,1 次 forward pass)      │
└─────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────┐
│ Step 2: 主模型验证                                │
...
│ Main Model: 并行验证                           │
│ Output: [t0, t1, t2, t3', t4', t5', t6', t7', t8']  │
│ Time:   ~50ms (大模型,1 次 forward pass)        │
└─────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────┐
│ Step 3: 比对结果                                  │
...
总时间:
  无投机采样: 100 × 50ms = 5000ms
  投机采样: 10ms + 50ms = 60ms (第一步) + ...
```
...
|------|---------|-------|--------|--------|--------|
| **Speculative Decoding** | 小型/量化 | 1.5-2x | 60-70% | 中 | ⭐⭐⭐⭐ |
| **Assisted Decoding** | 小型/量化 | 1.5-2x | 60-70% | 中 | ⭐⭐⭐⭐ |
| **Lookahead Decoding** | 多个小型 | 2-2.5x | 70-80% | 高 | ⭐⭐⭐ |
| **Eagle 3** | QAT 训练 | 2-3x | 80-90% | 中 | ⭐⭐⭐⭐⭐ |
...
| **推理速度** | 快 | 中 | 快 |
| **显存占用** | 小 | 大 | 小 |
| **接受率** | 60-70% | 70-75% | 80-90% |
| **实现复杂度** | 低 | 低 | 高 |
| **加速比** | 1.5-2x | 1.8-2.2x | 2-3x |
| **推荐度** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
...
```
无投机采样:
  100 tokens × 50ms/token = 5000ms

投机采样 (speculation_len = 10):
  初始 10 tokens: 1 × (10ms + 50ms) = 60ms
  剩余 90 tokens: 9 × (10ms + 50ms) = 540ms
  总时间: 600ms

加速比: 5000ms / 600ms = 8.3x 🎉
```
...
投机采样 (speculation_len = 10, accept_rate = 70%):
  成功投机: 7 次 (10 tokens × 0.7)
  失败投机: 3 次 (10 tokens × 0.3)
...
**关键洞察**:
```
加速比 ≈ 1 + speculation_len × accept_rate

例如:
  speculation_len = 10, accept_rate = 0.7
  → 加速比 ≈ 1 + 10 × 0.7 = 8x

实际加速比会略低 (因为草稿模型不是完全免费)
```
...
影响:
  - 草稿模型是大模型的 1/10 速度 → 投机开销 10%
  - 草稿模型是大模型的 1/20 速度 → 投机开销 5%
...
**性能数据**:
```
场景 1: 文章生成 (1000 tokens)
  无投机: 50s
  有投机 (Eagle 3): 18s
  加速比: 2.8x

场景 2: 代码生成 (500 tokens)
  无投机: 25s
  有投机 (Eagle 3): 10s
  加速比: 2.5x
...
**vLLM 暂不支持投机采样** (v0.6.x):
```
注意: vLLM v0.6.x 尚未原生支持投机采样

替代方案:
  - 使用 SGLang (原生支持 Eagle 3)
  - 等待 vLLM Speculators 成熟
```
...
场景 2: 生产环境 (追求性能)
  → 使用量化主模型 (Llama-2-70B-INT4)
  → 或使用 Eagle 3 (如果有兼容 checkpoint)

场景 3: 自定义需求
  → 训练专门的草稿模型 (使用 vLLM Speculators)
```
...
## 9.7 实战: Eagle 3 with SGLang ⚠️ NVIDIA 官方支持

> **💡 工业界实践** (来源: NVIDIA Model Optimizer Blog)
>
> **核心洞察**: Eagle 3 是 NVIDIA Model Optimizer 团队训练的投机采样 checkpoint,通过 QAT 训练优化,在 SGLang 中可直接使用,实现 2-3 倍的生成速度提升。
...
Latency 改善: TTFT 降低 40%
Throughput 提升: TPS 提升 2.5 倍
...
### 9.7.7 实战: vLLM Speculators v0.3.0 - 端到端 Eagle 3 训练 ⭐💡

> **💡 2025 年技术趋势** (来源: vLLM Official Blog - 2025/12/13)
>
> **核心洞察**: vLLM Speculators v0.3.0 提供了完整的端到端 Eagle 3 训练支持,从离线数据生成到模型训练再到推理部署,填补了开源生态在投机采样训练方面的空白。
...
- Eagle 3 是当前最佳选择 (NVIDIA 官方支持,2-3x 加速)
- 草稿模型的选择是性能的关键
- 长文本生成是投机采样的最佳场景
