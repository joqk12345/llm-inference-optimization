...
> - **可部署性**：量化经常把“放不下”变成“能上线”，尤其对 70B+ 与长上下文场景
> - **质量风险**：精度损失没有免费的午餐，必须用任务级回归集与灰度/回滚把风险变成可控成本

## 简介

在前面的章节中，我们学习了 GPU、KV 与调度。但这些系统优化经常会撞到同一堵墙：**模型太大，显存不够**。70B 级别模型在 FP16/BF16 下单卡通常放不下；就算放得下，KV 与并发也会把你推回容量瓶颈。

**量化 (Quantization)** 是解决这个问题的核心武器。通过降低模型权重的数值精度,我们可以:
- 将 70B 模型从 140GB 压缩到 35GB (INT4)
- 在单张 GPU 上运行更大的模型
- 提升推理速度 (INT8 比 FP16 快 2-3 倍)
- 降低硬件成本 (更小的 GPU 或更少的 GPU)

但量化也带来挑战：精度损失、训练/推理不一致、不同框架的支持差异、以及“跑得起来但回归解释不了”的工程痛点。本章的重点不是让你记住名词，而是让你形成一套可执行流程：

- **背景**：为什么量化几乎是推理降本的第一刀？
- **决策**：PTQ/QAT/AWQ/GPTQ/FP8/INT4，什么时候该选哪类？
- **落地**：如何在同一套指标口径下做 A/B（吞吐、P95/P99、单位成本、任务级质量）？
- **踩坑**：哪些回归最常见，如何通过回归集与灰度把它们关进笼子？

**学完本章，你将能把量化当作一条可运营的生产流程：可测、可灰度、可回滚，而不是“一次性把精度降下去赌运气”。**

---

## 8.1 量化基础

**背景**：量化之所以常被当作推理降本的第一刀，是因为它同时作用于两条最贵的成本线：权重占用（容量）与权重/激活的访存量（带宽）。你不是“把数字变小”，而是在改变推理系统的资源画像。

**决策**：这一节先建立两条底线：

- 量化节省显存的收益很好算，但速度收益与质量损失都强依赖硬件与实现。
- 量化不是一次性操作，生产落地需要“可测、可灰度、可回滚”的流程。
...
权重占用的“下界”由参数量和字节数决定（忽略量化元数据与框架开销）：
...
FP16 → INT8:
  ...
  速度: 可能更快（取决于硬件支持与内核实现，不保证）
  精度: 可能有损（需要以任务级回归集评估）

FP16 → INT4:
  权重显存: 2 bytes/param → 0.5 byte/param (节省约 75%)
  速度: 可能更快（同样取决于实现；有时会被解码/反量化开销抵消）
...
| 格式 | 显存占用 | 速度 | 精度损失 | 适用场景 |
|------|---------|------|---------|---------|
| **FP32** | 100% | 1x | 0% | 训练 |
| **FP16** | 50% | 1.5x | <0.1% | 推理标准 |
| **BF16** | 50% | 1.5x | <0.1% | 推理标准 |
| **INT8** | 25% | 2.5x | 0.5-1% | 生产推理 |
| **INT4** | 12.5% | 3x | 1-3% | 极限压缩 |
| **FP8** | 25% | 2x | <0.5% | 未来方向 |
| **FP4** | 12.5% | 4x | 1-5% | 研究中 |
...
**背景**：你会在工程讨论中听到很多名词（PTQ/QAT/AWQ/GPTQ/QLoRA/FP8…）。把它们放到同一张图里看，最清晰的区分是：量化发生在训练后还是训练中？量化误差是否被训练过程“适配”过？

**决策**：选方法时先问三件事：

- 你是否能承受训练成本？如果不能，优先 PTQ。
- 你是否对质量极敏感？如果是，考虑 QAT 或更温和的量化配置。
- 你是否追求极限压缩？如果是，准备好更严格的回归与更复杂的部署链路。
...
**背景**：格式决定了两件事：你压缩的是权重还是权重+激活？你依赖哪条硬件路径（张量核心/整数指令/专用内核）？

**决策**：不要先争论“哪种最好”，先确定你的瓶颈：

- 容量瓶颈：优先权重量化（INT8/INT4/FP8 等）
- 带宽瓶颈：优先减少访存量（低比特 + 更好的内核）
- 质量瓶颈：优先做温和量化或上 QAT，并用回归集约束
...
| **稳定性** | 高 | 低 |
| **性能** | 快 | 理论更快 |
| **硬件支持** | 广泛 | 需要 Blackwell |
| **生态** | 成熟 | 发展中 |
| **推荐度** | ⭐⭐⭐⭐ | ⭐⭐⭐ (未来) |

**选择建议**:
- **当前**: INT4 (生态成熟,稳定可靠)
- **未来**: FP4 (理论性能更高,Blackwell 原生支持)
...
**背景**：量化不是“理论公式”，最终要落到框架支持与内核实现上。相同的位宽在不同框架里可能表现完全不同。

**落地**：选框架时建议同时看：

- 支持哪些格式（权重/激活、group size、对称/非对称）
- 端到端指标（TTFT/TPOT/P95/P99/单位成本）而不是只看 weights 体积
- 运维能力（可观测、可回滚、兼容模型更新）
...
Llama-2-7B INT4 vs FP16:
  显存: 35GB vs 140GB (4x 节省)
  速度: 120 tokens/s vs 40 tokens/s (3x 提升)
  精度: 损失 1.2%
```
...
**背景**：在长上下文与高并发场景里，KV 往往会比权重更早成为显存主导项。只量化权重，有时只能解决“能跑”，解决不了“能扛并发/能跑长上下文”。

**决策**：KV 量化通常更敏感，建议从“先测再开”开始：

- 先在代表性长上下文回归集上评估质量
- 再在代表性并发负载下评估 TPOT 与尾延迟
- 最后再决定是否全量开启，以及是否需要分模型/分场景策略

### 8.5.1 为什么量化 KV Cache

**KV Cache 在长上下文下会迅速膨胀**（以下为示例口径，用于理解趋势，不代表官方配置）：

```
Llama-2-7B (序列长度 4096):
  模型权重: 13 GB (FP16)
  KV Cache: 4 GB (FP16)
  激活值: 2 GB
  ─────────────────────
...
Llama-2-7B (序列长度 32768):
  模型权重: 13 GB (FP16)
  KV Cache: 32 GB (FP16)
  ─────────────────────
  总计: 45 GB (超过 A100 40GB!)

KV Cache 占比: 32/45 = 71%
```
...
**背景**：实战部署的核心不是“能加载”，而是“上线可控”。量化模型的典型风险是：某些任务偶发回归，且难以解释；而线上一旦出问题，往往影响的是信任与业务指标。

**落地**：建议按“灰度四步法”推进：

1. 离线回归：任务级评测 + 长尾样本
2. 线上小流量：只放低风险请求，保留人工兜底
3. 扩大覆盖：监控失败率、重试率、投诉/差评
4. 全量与回滚预案：任何回归都能快速切回 FP16/BF16 或更温和量化
...
## 8.7 量化进阶: INT4 QAT 实战 ⚠️ SGLang 团队验证

**背景**：QAT（Quantization-Aware Training）的价值是让模型在训练阶段“适应量化噪声”，从而在更激进的低比特（如 INT4）下仍保持可用质量。它的代价是训练更复杂、数据与算力成本更高、工程链路更长。

**决策**：只有当 PTQ 无法满足质量要求，且业务收益足够支撑训练成本时，QAT 才值得投入。常见信号包括：

- 关键任务在 PTQ 下出现稳定回归（不是偶发）
- 你需要更低位宽以满足容量/成本约束（否则无法上线或无法扩展）
- 你有能力维护回归集、灰度与回滚（否则 QAT 只是把风险延后到线上）
...
### 8.7.5 实战案例（示例口径）：把“跨节点瓶颈”变成“单机可控”

这一节用一个**典型工程目标**来说明 QAT 的价值，而不是讨论某个具体公司的私有数据。

**目标**：后训练/RL rollout 场景下，跨节点通信成为瓶颈（吞吐低、尾延迟抖动、成本高）。团队希望把工作负载尽可能收敛到单机（或更少节点）上，以减少通信与系统复杂度。

**背景**：当模型很大时，你往往被迫做多机并行；但多机并行的上限常常由互连与通信决定，而不是由 GPU 峰值算力决定。于是你会看到一个反直觉现象：

- 模型更大、卡更多，但 rollout 吞吐并没有线性提升。
- 系统越复杂，稳定性与可观测性越差，调参成本更高。

**决策**：如果你已经验证 PTQ 无法满足质量（稳定回归），而你又确实需要更低位宽去降低“并行需求”，这时 QAT 才成为合理选项。

**落地**：把目标拆成三步，避免“一步到位”的大赌注：

1. 先把“量化可行性”测出来：在代表性任务回归集上验证 INT4 QAT 的质量底线。
2. 再把“系统收益”测出来：在小规模 rollout 上测 tokens/s、失败率、队列与尾延迟，验证是否真的减少了通信瓶颈。
3. 最后把“运营成本”算清楚：单机方案是否降低了故障定位成本、运维复杂度与冗余开销。

**指标口径**：不要用一句“更快/更准”结束讨论，至少要同时给出：

- 性能：rollout tokens/s、P95/P99、失败重试率
- 成本：每条轨迹成本、每个有效样本成本、峰值冗余比例
- 质量：任务级回归集、关键业务约束指标（拒答/越权/工具调用正确率等）
...
**决策**：当你要把低精度带进生产，优先投入“对齐工程”往往比投入“更激进的位宽”更划算：

- 先保证训练/推理使用同一类数值语义（scale、round、clamp、累加方式）
- 再谈更激进的 INT4/FP4

**落地**：把“对齐”当成一条 checklist，而不是靠经验猜：

- 校准数据是否代表真实分布（尤其是长上下文与长尾任务）
- 推理算子与训练 fake quant 是否一致
- 部署环境（GPU/驱动/框架版本）是否一致或至少可控

**指标口径**：对齐问题常表现为“均值没变但尾部坏了”，因此需要同时看：

- 任务级回归集的长尾样本
- 一致性（同输入多次输出的波动）
- 线上错误类型分布（工具调用、引用、结构化输出）
...
证据 (张博涵@浙大):
  - FP4 量化: 掉 10-20 个点
  - 需要 special clipping 和 correction
  - 推荐至少使用 FP8
...
| 任务类型 | 推荐格式 | 精度损失 | 说明 |
|---------|---------|---------|------|
| **LLM** | INT4/INT8 | 1-2% | 离散采样,容忍度高 |
| **Diffusion** | FP8/FP16 | <1% | 连续采样,误差累积 |
| **Recommendation** | INT8 | <1% | 类似 LLM |
| **RL** | INT4 (QAT) | 1-3% | 需要 train-infer 一致 |
...
**背景**：量化不会消失，它会和硬件一起演进。你的目标不是追逐名词，而是建立“可运营量化流水线”：每次模型升级都能重新量化、重新回归、重新灰度，把风险锁在流程里。

**建议**：把量化能力建设成三个可复用模块：

- 数据：校准集与回归集（覆盖长尾与关键任务）
- 工程：一键量化与一键对齐验证（可复现）
- 运营：灰度、回滚与指标看板（可控）
...
**时间表**:
```
2025: Blackwell 上市,FP8 成熟
2026: FP4/NVFP4 实验性支持
2027: FP4/NVFP4 生产就绪
2028+: 新的量化格式 (FP2?)
```
