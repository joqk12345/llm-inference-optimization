...
但真正落地时，FP4 更常见的用法不是“全模型一次性替换”，而是先放到收益最明确的路径上，例如 MoE expert 权重、attention indexer 或某些局部矩阵路径。DeepSeek-V4 这类路线给出的信号就是：低精度是否值得，不该先问格式本身，而该先问哪一条路径最能吃到它的收益、又最不容易伤到稳定性。
