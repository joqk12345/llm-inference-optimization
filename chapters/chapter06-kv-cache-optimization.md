...
- 多种 KV Cache 优化技术 (GQA、量化、共享、压缩式 attention 等)
...
到这里很容易产生一个误解：好像 KV 问题只要靠 `PagedAttention + Prefix Caching + KV 量化` 就讲完了。其实没有。它们固然重要，但分别只解决了 KV 问题的一个切面。

如果把这几年常见的方法放在一张图里看，KV 相关优化大致分成三层：

**第一层：管理层**

- 代表：`PagedAttention`、block allocation、eviction、Prefix Caching
- 核心问题：KV 该怎么分配、回收、复用，才能少碎片、少浪费、少重复计算

这层不改变模型看到的历史信息，只是在把同样的 KV 资产管理得更好。

**第二层：压缩层**

- 代表：`MQA`、`GQA`、`MLA`、KV 量化
- 核心问题：每个 token 的 K/V 表示能不能更小

这层改变的是“每个 token 需要多少存储”，但通常不直接改变“序列里一共有多少 KV 条目”。

**第三层：结构层**

- 代表：`Lightning Attention`、`Kimi Linear`、`DeepSeek-V4` 的 `CSA/HCA`
- 核心问题：随着序列继续拉长，是否还需要把所有历史 token 都以原样、等价的方式保留下来并参与后续 attention

这层才真正开始触碰一个更根本的问题：**KV 的增长速度本身能不能被改写。**

把这三层分开看非常重要，因为它们回答的是不同问题：

- PagedAttention 解决的是 **显存碎片和分配方式**
- Prefix Caching 解决的是 **跨请求复用**
- KV 量化解决的是 **存储精度**
- MQA/GQA/MLA 解决的是 **每个 token 的表示大小**
- Linear / 压缩 attention 解决的是 **历史 token 该如何被表示和访问**

这也是为什么 `DeepSeek-V4` 和 `MiniMax-01 / Kimi Linear` 虽然都常被放进“长上下文模型”一类里，但它们其实代表了两种不同的结构级思路：

- `MiniMax-01`、`Kimi Linear` 更偏 **线性化路线**：试图把 attention 复杂度从 full attention 的形式中解耦出来。
- `DeepSeek-V4` 更偏 **压缩 + 稀疏 + 局部保真路线**：试图减少需要被完整保留和访问的 KV 条目数量。

从工程角度看，这意味着 KV 相关优化正在从“单一缓存结构问题”变成“异构状态管理问题”。vLLM 的 hybrid KV cache、SGLang 的 ShadowRadix，以及 DeepSeek-V4 这类模型暴露出来的压缩 attention 结构，本质上都在做同一件事：把不同形态的状态统一纳入调度、缓存、复用和回收语义，而不是只盯着 block 分配本身。

**工程判断**：

- 如果你当前的主要问题是“明明还有显存却 OOM”或“短 prompt 也跑不稳”，先做管理层优化，不要一上来就追模型结构创新。
- 如果你已经被长上下文的显存与带宽压住，第二步通常是压缩层：GQA、MLA、KV 量化。
- 如果你的业务已经明显走向 128K、256K、1M 级上下文，结构层路线才会开始进入认真评估的范围。

换句话说，第6章这部分最想帮你建立的，不是“哪个方案最先进”，而是一个顺序感：

1. 先把现有 KV 管好。
2. 再把每个 token 的 KV 变小。
3. 最后才考虑是否要重写 attention / KV 结构本身。
