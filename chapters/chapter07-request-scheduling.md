...
在更前沿的模型里，调度器还会进一步碰到一个新问题: 它不只是决定“谁先跑”,还要协调“哪些状态先进入关键路径”。像 DeepSeek-V4 这类模型把压缩 attention、prefix reuse、spec decode metadata 和 PD 传输都揉进同一个运行时之后，调度器实际上开始承担状态编排的角色。
